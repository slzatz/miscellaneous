'''
This micropython script is for use with the FeatherWing doubler.  It displays songs that
are being scrobbled to the mqtt broker running in AWS EC2 and also the top bottom (A) increases the volume, 
the bottom button (C) decreases the volume and middle (B) play_pauses
The script also pings the broker to keep it alive
The other board includes a volume potentiometer and a play/pause button
'''
